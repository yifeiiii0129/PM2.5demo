...
if (!window.d3) missingDependencies.push("D3");
if (!window.d3?.geoRobinson) missingDependencies.push("D3 geographic projections");
if (!DATA) missingDependencies.push("mortality data");
if (missingDependencies.length) {
  const detail = `Required files are unavailable: ${missingDependencies.join(", ")}. Check your connection and retry.`;
  window.PM25_UI?.showError(detail);
...
const els = {
  ...
  clearCity: document.querySelector("#clearCity"),
...
const smallDeathsFmt = new Intl.NumberFormat("en-US", { minimumFractionDigits: 1, maximumFractionDigits: 1 });
...
function formatDeaths(value, correspondingRate = null) {
  if (value == null) return "--";
  if (value === 0 && correspondingRate > 0) return "<1";
  if (value > 0 && value < 0.1) return "<0.1";
  if (value > 0 && value < 10) return smallDeathsFmt.format(value);
...
function clearCitySelection(options = {}) {
  selectedCity = null;
  els.citySearch.value = "";
  populateCities();
  drawCountries();
  ...
  if (options.zoom !== false) {
    if (selectedCountry) zoomToCountry(selectedCountry.iso3);
    else resetMapView();
  }
}

function clearCountrySelection(options = {}) {
  ...
  drawCities();
  renderPanel();
  if (options.zoom !== false) resetMapView();
}
...
function populateCountries() {
  ...
  els.country.innerHTML = `<option value="">Select…</option>${sorted
  ...
  const rows = DATA.ageGroups
    .map((age) => {
      ...
      const rateInterval = metric
        ...
        : "";
      return `<div class="age-row"><strong>${age.label}</strong><span title="${rateInterval}">${formatRate(metric ? metric.currentRatePer100k : null)}</span><span>${formatDeaths(metric ? metric.currentDeaths : null, metric ? metric.currentRatePer100k : null)}</span><span>${formatPercent(metric ? metric.avoidableShare : null)}</span></div>`;
    })
    .join("");
...
function renderPanel() {
  ...
  els.currentDeaths.textContent = formatDeaths(
    metric ? metric.currentDeaths : null,
    metric ? metric.currentRatePer100k : null
  );
  ...
  els.avoidableDeaths.textContent = formatDeaths(
    metric ? metric.avoidableDeaths : null,
    metric ? metric.avoidableRatePer100k : null
  );
...
function initEvents() {
  ...
  els.countrySearch.addEventListener("input", updateClearButtons);
  els.citySearch.addEventListener("input", updateClearButtons);
  ...
  els.clearCountry.addEventListener("click", () => {
    if (selectedCountry) clearCountrySelection();
    else {
      els.countrySearch.value = "";
      updateClearButtons();
    }
  ...
  els.clearCity.addEventListener("click", () => {
    if (selectedCity) clearCitySelection();
    else {
      els.citySearch.value = "";
      updateClearButtons();
    }
...
async function init() {
  ...
  if (!json || !Array.isArray(json.features) || !json.features.length) {
    throw new Error("The boundary file is empty or malformed.");
  }
  geojson = {
    ...json,
...
init().catch((error) => {
  console.error("PM₂.₅ Mortality Explorer failed to initialize:", error);
  window.PM25_UI?.showError(error.message || "Check your connection and try again.");
});
